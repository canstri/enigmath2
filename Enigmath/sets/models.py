...
class SetManager(models.Manager):
    ...
    def filter_by_instance(self, instance):
        ...
        return super(SetManager, self).filter(content_type=content_type, object_id= instance.id)

    def filter_by_author(self, author):
        return super(SetManager, self).filter(user=author)
...
class Set(models.Model):
    ...
    def get_delete_url(self):
        return reverse("sets:delete", kwargs={"id": self.id})
...
class PassSet(models.Model):
    user = models.PositiveIntegerField(null = True)
    set_id = models.PositiveIntegerField(null = True)
